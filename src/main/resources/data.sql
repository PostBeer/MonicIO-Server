...
BEGIN
    IF (NEW IS DISTINCT FROM OLD) THEN
        INSERT INTO jpa.taskslog(project_id, project_name, task_id, task_name, status, changed_on)
        VALUES ((SELECT project_id FROM jpa.project_task WHERE jpa.project_task.task_id = NEW.id),
                (SELECT title FROM jpa.project WHERE jpa.project.id = (SELECT project_id FROM jpa.project_task WHERE jpa.project_task.task_id = NEW.id)),
